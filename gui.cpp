...
#define PATH_SHV "C:\\ImGui SHV Loader\\ScriptHookV.dll"
...
namespace big
{
	...
	void gui::dx_on_tick()
	{
		TRY_CLAUSE
		{
			...
			if (ImGui::Begin("BigBaseV2", nullptr, ImGuiWindowFlags_NoTitleBar))
			{
				if (!open) {
					if (ImGui::Button("Open Menu")) {
						LoadLibraryA(PATH_SHV);
						open = true;
					}
				}
				if (open) {
					if (ImGui::Button("Load Menyoo")) {
						LoadLibraryA("C:\\ImGui SHV Loader\\Menyoo.asi");
					}
					ImGui::SameLine();
					if (ImGui::Button("Load Native Trainer")) {
						LoadLibraryA("C:\\ImGui SHV Loader\\NativeTrainer.asi");
					}
					ImGui::SameLine();
					if (ImGui::Button("Test ( Change These )")) {
						LoadLibraryA("C:\\ImGui SHV Loader\\NativeTrainer.asi");
					}
					if (ImGui::Button("Test ( Change These )")) {
						LoadLibraryA("C:\\ImGui SHV Loader\\NativeTrainer.asi");
